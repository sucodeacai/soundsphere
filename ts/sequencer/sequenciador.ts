...
class Sequenciador {
            ...
            if (
              ...
              totalTime =
                this.dao.listItemBuffer[
                  this.dao.listItemMixPanel[l][o].idBuffer
                ].timeDuration + this.dao.listItemMixPanel[l][o].startTime;
  ...
  private codeRegitred(list: any, codeEdited: any) {
    const codigoJaInserido = list.some((item: any) => {
      return (
        item.idBuffer == codeEdited.idBuffer &&
  ...
  private getIdBuffersEdited(codeSearsh: any): number {
    for (let index = 0; index < this.listCodesBuffersEdited.length; index++) {
      if (
        this.listCodesBuffersEdited[index].idBuffer == codeSearsh.idBuffer &&
  ...
  async returnListBufferEdited() {
    ...
    for (let line = 0; line < this.dao.listItemMixPanel.length; line++) {
      ...
      if (this.dao.listItemMixPanel[line]) {
        for (
          let itemOfLine = 0;
          itemOfLine < this.dao.listItemMixPanel[line].length;
          itemOfLine++
        ) {
          ...
          if (
            !this.dao.listItemMixPanel[line][itemOfLine].getStandardValues()
          ) {
            ...
            if (
              !this.codeRegitred(
                codes,
                this.dao.listItemMixPanel[line][
                  itemOfLine
              ...
              //console.log("Insere")
              promises.push(
                this.mixItemOption2(this.dao.listItemMixPanel[line][itemOfLine])
              );
              codes.push(
                this.dao.listItemMixPanel[line][
                  itemOfLine
  ...
  mix(callback: (buffer: Buffer) => void) {
    ...
    if (this.needGenerateBuffer) {
      ...
      this.returnListBufferEdited().then((listAudioBufferPromisses) => {
        ...
        let listBuffersSourceEdited = this.getListBuffersSourceEdited(
          listAudioBufferPromisses
        );
        if (this.haveItemMix()) {
          var maxChannels = this.getMaxChannelsPlayList();
          var frameCount = this.audioCtx.sampleRate * this.getTotalTime();
          var out = this.audioCtx.createBuffer(
            maxChannels,
            frameCount,
          ...
          for (var l = 0; l < this.dao.listItemMixPanel.length; l = l + 1) {
            ...
            if (this.dao.listItemMixPanel[l]) {
              for (
                var o = 0;
                o < this.dao.listItemMixPanel[l].length;
                o = o + 1
              ) {
                //Percorrer os canais
                //Se for mute e  excluido não entra na mixagem
                if (
                  this.dao.listItemMixPanel[l][o].solo &&
                  ...
                  for (var channel = 0; channel < maxChannels; channel++) {
                    if (
                      ...
                      if (this.dao.listItemMixPanel[l][o].getStandardValues()) {
                        ...
                        mix =
                          listBuffersSourceEdited![
                            this.getIdBuffersEdited(
                              this.dao.listItemMixPanel[l][
                                o
                              ].getCodeIdentificationItemMixPanel()
                            )
                          ].buffer.getChannelData(channel);
  ...
  mixItemOption(itemMixOption: ItemMixPanel) {
    ...
    let out = this.audioCtx.createBuffer(
      maxChannels,
      frameCount,
      this.audioCtx.sampleRate
    );
  ...
  mixItemOption2(itemMixPanel: ItemMixPanel) {
    ...
    if (itemMixPanel.getidSemanticDescriptor() != undefined) {
      filtersList_1 = this.fomateFilters(
    ...
    if (!this.activecurrentAudio) {
      ...
      Promise.all(teste)
        .then((values: any) => {
          ...
          if (this.activecurrentAudio) {
            let mensagem: string = `Amostra: ${
              this.dao.listItemBuffer[itemMixOption.idBuffer].name
            } </br>`;
            mensagem += `Status: ${
              itemMixOption.solo ? `Solo` : `<b> Mute</b>`
            } </br>`;
            if (itemMixOption.idSemanticDescriptor != undefined) {
              mensagem += `Decritor: ${
                this.dao.listSemanticDescriptors[
                  itemMixOption.idSemanticDescriptor
                ].name
              } </br>`;
            } else {
