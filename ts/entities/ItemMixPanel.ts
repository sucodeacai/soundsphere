...
*/
class ItemMixPanel {
  tag_dimension: string | undefined = undefined;
  tag_intensity: string | undefined = undefined;
  ...
  drawPadrao(painel: any, y: number, height: number, yDescritor: number) {
    ...
    if (this.solo) {
      if (painel.drawGradient) {
        ...
        const gradiente = painel.ctxCanvas.createLinearGradient(
          this.x + this.width / 2,
          y,
          this.x + this.width / 2,
          y + height
        ...
        for (var i = 0; i < 3; i++) {
          if (this.codeSemanticDescriptor![i] != undefined) {
            ...
            painel.ctxCanvas.strokeText(
              this.codeSemanticDescriptor![i],
              this.x + 3,
              yDescritor + 10 + i * verticalSpacing
            );
            painel.ctxCanvas.shadowBlur = 0;
    ...
    if (this.tag_dimension != undefined) {
      if (painel.drawDimension) {
        ...
        painel.ctxCanvas.arc(
          this.x + 56,
          yDescritor + verticalSpacing - 2,
          9,
          0,
          2 * Math.PI
        );
    ...
    if (this.descriptiveIcon != undefined) {
      ...
      if (painel.drawFood) {
        painel.ctxCanvas.drawImage(
          painel.pageSoundSphereHome.getImgDescriptiveIcon(
            this.descriptiveIcon
          ),
          this.x + 20,
          this.y - 15,
          30,
  ...
  getLightnessByVolume(porcentagem: number) {
  ...
  hexToHsl(hex: string): [number, number, number] {
    ...
    let h: number = 0, // Inicializa h com um valor padrão
      s: number,
      l: number = (max + min) / 2;
