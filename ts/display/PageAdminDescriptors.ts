class PageAdminDescriptors extends SimplePage {
  ...
  generateAttributes() {
    ...
    let filter =
      this.dao.listSemanticDescriptors[
        this.positionDescriptorSelected
      ].getFilters();
    let conteudo = `
    ...
    filter.forEach((element, index) => {
      ...
      <div style="margin-bottom:1px" class="ui  header">
        ...
        <span id="span1${element.type}${index}" ${
        element.status != true ? `class="span1Line" ` : ""
      }>
    
          <span ${
            element.status != true ? `class="span2Line" ` : ""
          } id="span2${element.type}${index}">${index} - ${element.name}</span>
        </span>
     
        <div class="mini ui buttons">
          <button data-value="${index}" class="mini ui positive ${
        index == 0 ? " disabled " : " "
      } button">
            <i  data-value="${index}" class="angle up icon"></i>
          </button>
          <div class="or" data-text="ou"></div>
          <button data-value="${index}" class="mini ui orange ${
        index ==
        this.dao.listSemanticDescriptors[
          this.positionDescriptorSelected
        ].getLenght() -
        ...
        <div class="ui labeled input field">
          <div data-type="${element.type}${index}" id="divCheck${
        element.type
      }${index}" class="ui toggle checkbox ${
        element.status == true ? `checked` : ""
      }">
            <input data-type="${element.type}${index}" id="status${
        element.type
      }${index}" ${
        element.status == true ? `checked="checked" ` : ""
      } type="checkbox">
            <label class="labelCheck" for="status${element.type}${index}"> ${
        element.status == true ? `ativado` : "desativado"
      }
            </label>
          </div>
        </div>
      ...
      <div class="three fields">
        ...
        <div class="field">
          ...
          <input id="frequency${element.type}${index}" value="${
                element.frequency
              }" type="number" min="1" max="24000" ${
                element.status == true ? "" : "disabled"
              }/>
        </div>`
            : ""
        }
        ...
        <div class="field">
          <label>Q</label>
          <input id="Q${element.type}${index}" value="${
                 element.Q
               }" type="number" min="1" max="24000" ${
                 element.status == true ? "" : "disabled"
               } />
        ...
        <div class="field">
          <label>Ganho</label>
          <input id="gain${element.type}${index}" value="${
              element.gain
            }" type="number" min="1" max="24000" ${
              element.status == true ? "" : "disabled"
            } />
  ...
  setSettingsActions() {
    ...
    document
      ...
      .on("mouseout", (e: JQueryEventObject) => {
        this.sequenciador.stopOneSound();
      });
    document
      .getElementById("buttonPlayFilter")
      .on("mouseover", (e: JQueryEventObject) => {
        this.setAttributesFilter();
        // console.log("PageSoundSphereHome  play")
        this.sequenciador.playOneSoundDescitor(
          this.controlSelectedItemBuffer,
          function () {},
          this.dao.listSemanticDescriptors[
            this.positionDescriptorSelected
          ].getFilters()
  ...
  setSettingsAttributes() {
    ...
    $(".ui.compact.button.red").on("click", (e: JQueryEventObject) => {
      $(e.target).closest("[data-id]")?.classList.add("removed");
      setTimeout(() => {
        this.setAttributesFilter();
        ...
        this.dao.listSemanticDescriptors[
          this.positionDescriptorSelected
        ].removeFilter($(e.target).data("value"), () => {
          this.render();
        });
      }, 800);
    });
    $(".button.positive").on("click", (e: JQueryEventObject) => {
      $(e.target).closest("[data-id]")?.classList.add("subir comando");
      $(
        `#container${$(e.target).closest("[data-id]").data("order") - 1}`
      )?.classList.add("descer");
      setTimeout(() => {
        this.setAttributesFilter();
    ...
    $(".button.down").on("click", (e: JQueryEventObject) => {
      $(e.target).closest("[data-id]")?.classList.add("descer comando");
      $(
        `#container${$(e.target).closest("[data-id]").data("order") + 1}`
      )?.classList.add("subir");
      setTimeout(() => {
        ...
        this.dao.listSemanticDescriptors[
          this.positionDescriptorSelected
        ].downFilter($(e.target).data("value"), () => {
          this.render();
        });
      }, 500);
    });
    (<any>$(".ui.checkbox")).checkbox({
      ...
      beforeUnchecked: function () {
        $(`#span1${$(this).data("type")}`)?.classList.add("span1Line");
        $(`#span2${$(this).data("type")}`)?.classList.add("span2Line");
...
<div class="actions">
  ...
  addFilter() {
    ...
    if (this.newFilterModal!.frequency !== undefined) {
      ...
      // console.log(<number>$(`#addFrequency${this.newFilterModal!.type}`).val())
      this.newFilterModal!.frequency = <number>(
        $(`#addFrequency${this.newFilterModal!.type}`).val()
      );
    }
    if (this.newFilterModal!.Q !== undefined) {
      this.newFilterModal!.Q = <number>(
        $(`#addQ${this.newFilterModal!.type}`).val()
      );
    }
    if (this.newFilterModal!.gain !== undefined) {
      this.newFilterModal!.gain = <number>(
        $(`#addGain${this.newFilterModal!.type}`).val()
      );
    }
    this.dao.listSemanticDescriptors[this.positionDescriptorSelected].addFilter(
      this.newFilterModal!
    );
  }
  setAttributesFilter(): void {
    ...
    filter.forEach((element, index) => {
      ...
      if (element.frequency != undefined) {
        element.frequency = <number>(
          $(`#frequency${element.type}${index}`).val()
        );
      }
      ...
      element.status = $(`#divCheck${element.type}${index}`).hasClass(
        "checked"
      );
    });
  }
  ...
  generateHTMLFilter() {
    ...
    <div class="field">
      <label>Frequency</label>
      <input id="addFrequency${this.newFilterModal.type}" value="${
            this.newFilterModal.frequency
          }" type="number" min="1" max="24000" ${
            this.newFilterModal.status == true ? "" : "disabled"
          }/>
    </div>`
        : ""
    } ${
    ...
    <div class="field">
      <label>Q</label>
      <input id="addQ${this.newFilterModal.type}" value="${
            this.newFilterModal.Q
          }" type="number" min="1" max="24000" ${
            this.newFilterModal.status == true ? "" : "disabled"
          } />
    </div>`
    ...
    <div class="field">
      <label>Ganho</label>
      <input id="addGain${this.newFilterModal.type}" value="${
            this.newFilterModal.gain
          }" type="number" min="1" max="24000" ${
            this.newFilterModal.status == true ? "" : "disabled"
          } />
    </div>`
