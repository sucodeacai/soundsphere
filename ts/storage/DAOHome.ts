class DAOHome extends DAO {
  ...
  cloneItemMix(itemMixPanel: ItemMixPanel) {
  ...
  pushItemMixPanel(itemMixPanel: ItemMixPanel) {
    this.sessionControl.addEventItemMixPanel(
      new EventItemMixPanel(this.cloneItemMix(itemMixPanel), EventsCRUD.INSERT)
    );
      ...
      if (this.listItemMixPanel[linha]) {
        for (
          ...
          if (this.listItemMixPanel[linha][index].excluded == false) {
            this.deleteItemMixPanel(
              this.listItemMixPanel[linha][index],
              linha,
              true
            );
          }
    ...
    for (let index = 0; index < this.listItemMixPanel[linha].length; index++) {
      if (this.listItemMixPanel[linha][index].id == itemMixPanel.id) {
        ...
        if (deleteFromRestartPainel) {
          this.sessionControl.addEventItemMixPanel(
            new EventItemMixPanel(
              this.listItemMixPanel[linha][index],
              EventsCRUD.RESET_PAINEL
            )
          );
        } else {
          this.sessionControl.addEventItemMixPanel(
  ...
  eraseModifiers(itemMixPanel: ItemMixPanel) {
    this.sessionControl.addEventItemMixPanel(
      new EventItemMixPanel(
        this.cloneItemMix(itemMixPanel),
        EventsCRUD.ERASE_MODIFIERS
      )
    );
  }
  updateItemMixPane(itemMixPanel: ItemMixPanel) {
    this.sessionControl.addEventItemMixPanel(
      new EventItemMixPanel(this.cloneItemMix(itemMixPanel), EventsCRUD.UPDATE)
    );
