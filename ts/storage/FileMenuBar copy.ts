class FileMenuBarBkp extends FileWav {
  ...
  constructor(
    ...
    //   }
    // });
    const filesWav = document.getElementById("filesWav") as HTMLInputElement;
          ...
          if (navigator.userAgent.match(/Android/i)) {
            ...
            for (let i = 0; i < files.length; i++) {
              ...
              if (
                ...
                this.listNamesInvalid.push(
                  `${file.name}: -Arquivo já carregado/ File already loaded`
                );
              } else {
                console.log("==Arquivo repetido");
                this.listNamesInvalid.push(
                  `${file.name}: -Formato inválido/ Invalid format`
                );
              }
            }
          }
